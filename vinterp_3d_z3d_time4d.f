C NCLFORTSTART
      SUBROUTINE DINTERP4DZ(V4D,Z,HEIGHT,V3D,NX,NY,NZ,NT)
      IMPLICIT NONE
      INTEGER NX,NY,NZ,NT
      DOUBLE PRECISION V4D(NT,NZ,NX,NY),V3D(NT,NX,NY)
      DOUBLE PRECISION Z(NT,NZ,NX,NY)
      DOUBLE PRECISION HEIGHT(NT,NX,NY)
C NCLEND
...
c set offset appropriately
      ...
      IP = 0
      IM = 1
      IF (Z(1,1,1,1).GT.Z(1,NZ,1,1)) THEN
          IP = 1
          IM = 0
      END IF
c      DO IT=1,NT
c        IF (Z(IT,1,1,1).GT.Z(IT,NZ,1,1)) THEN
c          IP = 1
c          IM = 0
c        END IF
...
C Initialize to missing.  Was initially hard-coded to -999999.
              ...
              DO WHILE ((.NOT.INTERP) .AND. (KP.GE.2))
                  ...
                  IF (((Z(IT,KP-IM,I,J).LE.HEIGHT(IT,I,J)).AND.
     +                (Z(IT,KP-IP,I,J).GT.HEIGHT(IT,I,J)))) THEN
                      W2 = (HEIGHT(IT,I,J)-Z(IT,KP-IM,I,J))/
     +                     (Z(IT,KP-IP,I,J)-Z(IT,KP-IM,I,J))
                      W1 = 1.D0 - W2
                      V3D(IT,I,J) = W1*V4D(IT,KP-IM,I,J) + 
     +                              W2*V4D(IT,KP-IP,I,J)
                      INTERP = .true.
                  END IF
                  KP = KP - 1
