...
c    SFTMK(MJX,MIY): Near-surface temperature (e.g., 2m) (K)
c    SFQVP(MJX,MIY): Near-surface mixing ratio (e.g., 2m) (kg/kg)
...
c
      ...
      DOUBLE PRECISION SFTMK(MIY,MJX)
      DOUBLE PRECISION SFQVP(MIY,MJX)
      DOUBLE PRECISION CAPE(MIY,MJX)
      DOUBLE PRECISION CIN(MIY,MJX)
      DOUBLE PRECISION LCL(MIY,MJX)
      DOUBLE PRECISION LFC(MIY,MJX)
      DOUBLE PRECISION EL(MIY,MJX)
      DOUBLE PRECISION LCLT(MIY,MJX)
      DOUBLE PRECISION ELT(MIY,MJX)
      DOUBLE PRECISION PGHT(MIY,MJX)
      DOUBLE PRECISION CMSG
      CHARACTER*(*) PSAFILE
C NCLEND
c Local variables
      INTEGER I,J,K,ILCL,IUP,KEL,KK,KLCL,KLEV,KLFC,KMAX,KPAR,KPAR1,KPAR2
      DOUBLE PRECISION DAVG,ETHMAX,Q,T,P,E,ETH,ETHSFC,TLCL,ZLCL
...
c      Find parcel with max theta-e in lowest 3 km AGL.
                 Q = MAX(SFQVP(I,J),1.D-15)
                 E = SFQVP(I,J)*SFP(I,J)/ (EPS+SFQVP(I,J))
                 TLCL = TLCLC1/ (LOG(SFTMK(I,J)**TLCLC2/E)-TLCLC3) +
     +                    TLCLC4
c                set ETHMAX to surface theta-e
                 ETHMAX = SFTMK(I,J)* (1000.D0/SFP(I,J))**
     ...
     +                      (1.D0+THTECON3*Q))
                 ...
                 DO K = MKZH,1,-1
c                top down approach
                   IF (((GHT(K,I,J)-TER(I,J).LE.3000.D0)
     +                .AND.(GHT(K,I,J)-TER(I,J).GE.0.D0))
     +                .AND.((PRS(K,I,J)).LE.(SFP(I,J)))) THEN 
...
c                Find avg parcel properties in lowest 100mb.
                 ...
                 DO K = MKZH,1,-1
c                top down approach
                      IF ((PRS(K,I,J).GE.((SFP(I,J)-100.D0)))) THEN
                          P = PRS(K,I,J)
                          TOTPRS = TOTPRS + P
                          Q = MAX(QVP(K,I,J),1.D-15)
     ...
     +                         (GAMMA* (1.D0+GAMMAMD*Q))
                      ...
                      END IF
                  END DO
                  SFTH = SFTMK(I,J)* (1000.D0/SFP(I,J))**
     +                         (GAMMA* (1.D0+GAMMAMD*SFQVP(I,J)))
                  KPAR1 = INT(KLEV/KK)
                  KPAR2 = INT(KLEV/KK)
c                  UPP computes MLCAPE as lifting from level of lyr avg
                  PAVG = (TOTPRS+SFP(I,J))/(KK+1)
                  PRSPARI = PAVG
                  GHTPARI = (GHT(KPAR1,I,J)+TER(I,J))/2
c
                  QVPPARI = (TOTQVP+SFQVP(I,J))/(KK+1)
...
c
                   CAPE(I,J) = MAX(PAREA(KEL),0.0D0)
                   CIN(I,J) = MAX(ABS(NAREA(KEL)),0.0D0)
                   IF (CAPE(I,J).EQ.0.0D0) CIN(I,J) = 0.0D0
c
c   In V6.1.0 of NCL, we added a _FillValue attribute to the return
c   value of this function. At that time we decided to change -0.1 
c   to a more appropriate missing value, which is passed into this 
...
C  meters AGL
              IF (CAPE(I,J).GT.0.D0) THEN

                 LFC(I,J) = (ZREL(KLFC)+(GHTPARI)) - TER(I,J)
                 EL(I,J) = (ZREL(KEL)+(GHTPARI)) - TER(I,J)
                 LCLT(I,J) = TLCL-273.15D0
                 ELT(I,J) = PTMP(KEL)-273.15D0
              ELSE
                 LFC(I,J) = -9999.D0
                 EL(I,J) = 0.D0
                 LCLT(I,J) = TLCL-273.15D0
                 ELT(I,J) = SFTMK(I,J)-273.15D0 
              END IF

              IF (LCL(I,J).LT.0.D0) LCL(I,J) = 0.D0
              IF (LFC(I,J).LT.0.D0) LFC(I,J) = 0.D0
              IF (EL(I,J).LT.0.D0) EL(I,J) = 0.D0
c
          END DO
      END DO
c
