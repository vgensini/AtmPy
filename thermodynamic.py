...
def frzlvl(t, z):
    '''
    Calculate the freezing level (assumes a 4-D array w/ second dim as vertical)
    '''
    z = numpy.ma.masked_where(t > 0., (z))
    lower_indices, upper_indices = numpy.ma.notmasked_edges(z,axis=1)
    frzlvl = z[lower_indices].reshape(z.shape[0],z.shape[2],z.shape[3])
    return frzlvl
...
def cape_plev_3D(prs_mb,tmp,mixr,hgt,ter,psfc_mb,sfc_t,sfc_mixr,parcel):
    ...
    ** we will check for this below by sorting pressure  **
    ...
    plevtest = prs_mb[:,0,0]
    sortedplev = numpy.sort(plevtest)
    if numpy.array_equal(plevtest,sortedplev) == False:
...
def cape_plev_4D(prs_mb,tmp,mixr,hgt,ter,psfc_mb,sfc_t,sfc_mixr,parcel):
    ...
    ** order needs to be top down for vertical dimension **
    ...
    if numpy.array_equal(plevtest,sortedplev) == False:
        ...
        mixr = mixr[:,::-1,:,:]
        hgt = hgt[:,::-1,:,:]
    prs_mb = numpy.swapaxes(prs_mb,0,1)
    tmp = numpy.swapaxes(tmp,0,1)
    mixr = numpy.swapaxes(mixr,0,1)
    hgt = numpy.swapaxes(hgt,0,1)
